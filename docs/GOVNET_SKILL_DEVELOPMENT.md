...
`canonical_bytes(VoteVector)` — exact server-side implementation at
`crates/emg-protocol/src/voting.rs:79-86` (a verbatim copy lives in
`crates/emg-core/src/types.rs:462` for cross-crate use; bytes are
identical):

```
[u32 LE: n] [16 bytes: rust_decimal::Decimal::serialize()] × n
```

- 4 bytes: vector length as `u32` little-endian.
- For each value: raw 16-byte output of `rust_decimal::Decimal::serialize()`.
  No bincode envelope. No length prefix per element. No trailing-zero
  stripping (the `Decimal`'s `scale` is preserved as-stored).
- Empty vector → exactly `[0x00, 0x00, 0x00, 0x00]` (just the length prefix).

`Decimal::serialize()` 16-byte layout (little-endian everywhere):

```
byte  0 : reserved (0x00)
byte  1 : reserved (0x00)
byte  2 : scale (0..28)
byte  3 : flags (0x00 positive, 0x80 negative)
bytes 4-7  : mantissa low  32 bits LE
bytes 8-11 : mantissa mid  32 bits LE
bytes 12-15: mantissa high 32 bits LE
```

Pinned fixtures (rust_decimal `~1.41`, locked by
`canonical_bytes_fixture_locks_decimal_serialize_layout` in
`voting.rs`):

| Decimal value | 16-byte hex |
|---|---|
| `0` | `00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00` |
| `1` | `00 00 00 00 01 00 00 00 00 00 00 00 00 00 00 00` |
| `0.5` | `00 00 01 00 05 00 00 00 00 00 00 00 00 00 00 00` |
| `0.25` | `00 00 02 00 19 00 00 00 00 00 00 00 00 00 00 00` |
| `0.123456789012345` | `00 00 0f 00 79 df 0d 86 48 70 00 00 00 00 00 00` |

Cross-implementation skills MUST verify their encoder produces these
exact bytes before submitting any signed vote — a single off byte
yields a digest mismatch and a `BUSINESS_VOTE_VOTE_HASH_MISMATCH`
rejection.
...
| GET | `/v1/epochs/{id}/results` | Settlement aggregate. `{ market_id, v_vector, w_vector, p_open_vector, total_gov_tokens }` — vectors ordered by worknet position ASC, decimals as STRINGS at scale 18 |
...
| GET | `/v1/leaderboard/epistemic?market_id={id}&limit={n}` | Epistemic-score ranking |
...
| GET | `/v1/orders` | List my orders. Query: `status`, `worknet_id`, `cursor`, `limit`. SQL-pushed filters (M-ORDERS); pagination is index-backed via `idx_orders_filtered_list` |
...
| `IDEMPOTENCY_KEY_REUSE` | 422 | Same `X-Idempotency-Key` reused with a different request body. Per spec §9.4 + ADR-014 §8 (resolution of the §9.5.1 inconsistency, post-H3 fix). Wire envelope includes `details.previous_hash`. | Surface "key already used for a different request"; tell user to pick a fresh key |
| `BUSINESS_REPORT_ALREADY_SUBMITTED` | 409 | `(market_id, worknet_id)` UNIQUE collision on `weekly_reports` — this WorkNet operator already submitted a report this epoch. Distinct from idempotency-key reuse (different code prefix) so clients can dispatch unambiguously. | Surface "this WorkNet has already submitted; one report per WorkNet per epoch"; do NOT auto-retry |
